...
Agents use the `subagent(...)` tool with `workflowScript` or `workflowScriptPath` for execution, and `action` for management, status, and control. `workflowArgs` supplies an optional frozen string-to-string map to a script as `args`. Humans often use the slash-command layer instead:

- `/run` — launch a single agent
- `workflowScript` / `workflowScriptPath` — the public script-delivery surfaces for sequence, parallelism, branching, retries, and aggregation
