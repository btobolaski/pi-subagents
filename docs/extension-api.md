...
- `spawn` accepts exactly one of `workflowScript` or `workflowScriptPath`, with optional `workflowArgs`, and is async-only: omit `async` or set `async: true`, omit `clarify`, and do not pass management `action` values. File-backed requests use the same confined-path, size, UTF-8, and string-map argument validation as the `subagent` tool. It goes through the same executor, so agent discovery, session attribution, configured spawn caps, child-safety depth, artifacts, and async status all behave the same.
